...
本特性在游戏画面顶层渲染屏幕空间 HUD 叠加层，显示金币计数和生命计数。HUD 从 Player 读取 `PlayerStats`（IAPI-009），以视口相对坐标 (3%, 3%) 锚定，数值变化时在当前帧内即时更新。所有文字使用带 1px 黑色描边的像素字体，图标为 8x8px 像素艺术。

## Design Alignment

将系统设计 §2.9 的完整内容复制于此。

- **Key types**: `HudRenderer` — 持有金币/心形图标纹理句柄、像素字体引用
- **Provides / Requires**:
...
|--------|-----------|---------------|----------------|--------|
| `HudRenderer::new` | `new() -> Self` | Macroquad 上下文已初始化（窗口已创建，GL 上下文活跃） | `coin_tex` 和 `heart_tex` 已加载为 8x8px 纹理；`font_size` 和 `icon_scale` 已设置默认值；返回就绪的 HudRenderer 实例 | 若纹理文件缺失或加载失败：宏 quad `load_texture` 返回默认 1x1 白色纹理（Macroquad 容错行为）；文本缺失时 `draw_text_ex` 回退至默认字体 |
...
**方法状态依赖**: 无 — HudRenderer 无内部状态机（仅持有不可变纹理句柄和渲染参数）。所有渲染行为仅由输入参数 `stats`、`viewport_w`、`viewport_h` 决定。纯函数式渲染通道，无状态转换。

**Design rationale**:
- `render` 方法使用 `&self`（不可变借用）而非 `&mut self`：HUD 渲染不修改内部状态，仅读取缓存的纹理句柄。这避免了与 PlayingState 其他渲染组件的不必要互斥。
...
|----------------|---------------------|---------------|----------------------|-------------------|-------------|
| 金币图标 | `draw_texture_ex(coin_tex, anchor_x, anchor_y, ...)` 位于 canvas 坐标 `(vp_w * 0.03, vp_h * 0.03)` | 每帧 PlayingState::render() 调用时 | 无状态变体 — 始终为 8x8px 金色像素图标 | 8x8px 源纹理，按 `icon_scale` 倍率放大至目标尺寸（默认 3x → 24x24px） | 预加载的静态纹理 `assets/coin.png` |
...
| 心形图标 | `draw_texture_ex(heart_tex, anchor_x, heart_icon_y, ...)` 位于金币行下方 4px | 每帧 PlayingState::render() 调用时 | 无状态变体 — 始终为 8x8px 红色像素图标 | 8x8px 源纹理，按 `icon_scale` 倍率放大至目标尺寸（默认 3x → 24x24px） | 预加载的静态纹理 `assets/heart.png` |
...
**Rendering technology**: Macroquad OpenGL 2D 即时模式（`draw_texture_ex` + `draw_text_ex`）
...
本特性创建 `src/systems/hud.rs` 中的 `HudRenderer` 结构体（文件已存在但为空）。该结构体持有两个 Macroquad `Texture2D` 句柄（`coin_tex`、`heart_tex`）以及渲染参数（`font_size: u16`、`icon_scale: f32`）。

`HudRenderer::new()` 在构造时通过 `macroquad::texture::load_texture("assets/coin.png")` 和 `macroquad::texture::load_texture("assets/heart.png")` 加载图标纹理。纹理文件需提前创建为 8x8px PNG 像素艺术资源。
...
- **纹理即代码 vs 外部文件**：选择将图标作为外部 PNG 文件（`assets/coin.png`、`assets/heart.png`），而非嵌入为 Rust 字节数组。理由：便于美术迭代、Macroquad 原生 `load_texture` 支持、与项目现有架构（`src/assets/` 目录预留）一致。若需零外部依赖打包，可在后续将纹理编译进二进制。
...
- **`src/assets/`** 目录为空 — 本特性将在此目录中放置 `coin.png` 和 `heart.png` 纹理文件。
...
| HUD-13 | UI/render | §Visual Rendering Contract 正向断言 (coin icon), §Visual Rendering Contract Element 1 | 加载 coin.png 纹理 → 调用 `render(stats, 480.0, 270.0)` → 检查 canvas 像素 | 金币图标锚定位置 `(14.4, 8.1)` 处存在非透明像素，颜色含金色调 (R>200, G>160, B<50) | 纹理加载失败（显示默认白色方块）、图标绘制位置偏移 |
...
| HUD-15 | UI/render | §Visual Rendering Contract 正向断言 (heart icon), §Visual Rendering Contract Element 3 | 加载 heart.png 纹理 → `render()` → 检查 canvas 像素 | 心形图标在金币行下方 4px 处绘制，颜色含红色调 (R>200, G<50, B<50) | 心形图标与金币图标重叠、颜色错误（加载了错误纹理） |
...
| HUD-21 | FUNC/error | §Interface Contract HudRenderer::new preconditions, §Implementation Summary §4 存量代码交互点 | coin.png 纹理文件缺失 → 调用 `HudRenderer::new()` → `load_texture` 返回 Macroquad 默认 1x1 白色纹理 | HudRenderer 构造成功（不 panic）、render() 正常完成；图标显示为白色方块而非金币图标，但游戏继续运行 | 纹理加载失败导致 panic 或 HudRenderer 构造失败阻断游戏启动 |
...
- [x] §2.N 中所有函数/方法都至少有一行 Test Inventory（HudRenderer::new: 由 HUD-13/15 隐式覆盖纹理加载; HudRenderer::render: HUD-01~HUD-20 全部直接或间接覆盖）
